...
import { TopPerformingPlan } from '../../../types/plans';

interface TopPlanCardProps {
  bestPerformingPlans: TopPerformingPlan[];
}

export default function TopPlanCard({ bestPerformingPlans }: TopPlanCardProps) {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-[#E5E7EB] space-y-4">
      <div>
        <h3 className="font-extrabold text-sm text-[#1F2937] uppercase tracking-wider">Top Performing Plans</h3>
        <p className="text-[11px] text-slate-400 font-semibold mt-0.5">Ranked by weekly subscriber volumes</p>
      </div>
          ...
          >
            <div className="flex items-center space-x-3.5 overflow-hidden">
              ...
              <div className="shrink-0 flex items-center justify-center">
                {getRankIndicator(plan.rank)}
              </div>
              <div className="overflow-hidden">
                <h4 className="font-extrabold text-xs text-[#1F2937] truncate">{plan.name}</h4>
            ...
            <div className="text-right shrink-0">
              ...
              <span className="block text-[8px] font-black text-[#16A34A] mt-0.5">+{plan.growthPercentage}% growth</span>
            </div>
          </div>
        ))}
      </div>
